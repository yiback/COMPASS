...
> **최종 업데이트**: 2026-03-25 (세션 33: 인프라 부채 정리 + 단계 2-3 역할별 대시보드 완료, 1460 tests)
...
└─────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  단계 1: 1-1 → 1-2 → ... → 1-9 → 통합 테스트 → 완료              │
├─────────────────────────────────────────────────────────────────┤
│  단계 1.5: 1.5-1 (LaTeX 렌더링) → 1.5-2 (도형 렌더링) → 완료     │
│  (단계 2 PDF 출력의 전제 조건 — 표시 품질 개선)                    │
├─────────────────────────────────────────────────────────────────┤
...
- ✅ 빌드 검증 (347 tests PASS, lint 0 errors, build 성공)

### 1-7. 기출 기반 AI 문제 생성 [F011] ✅

- ✅ Step 1: 타입 확장 + Zod 스키마 (PastExamContext, generateQuestionsRequestSchema, 369 tests PASS)
- ✅ Step 2: 프롬프트 빌더 — buildPastExamGenerationPrompt (TDD, 14 tests, 383 tests PASS)
- ✅ Step 3: Server Action + GeminiProvider 통합 (TDD, 21 tests, 404 tests PASS)
...
- ✅ 사용자 로그인 → 기출문제 업로드 → AI 문제 생성 플로우 E2E 테스트 (세션 26, Chrome DevTools MCP)
- ✅ 시험 생성 → 이미지 업로드 → AI 추출 → 편집 → 재분석 → 확정 E2E 테스트 (세션 26, Chrome DevTools MCP)
...
> **계획**: `docs/plan/dashboard-by-role.md` (v2) + `docs/plan/dashboard-tasks-detail.md`
> **작업량**: S (1일) → **완료** (세션 33, 1460 tests, E2E 통과)

- [x] 학원장 대시보드 (사용자/기출/문제 통계 + 최근 활동)
- [x] 강사 대시보드 (내 기출/문제/추출상태 + 최근 활동)
- [x] 학생 대시보드 (빈 상태 + 바로가기)
- [x] system_admin 대시보드 (전체 학원/사용자/기출/성취기준 통계)
- [x] ~~대시보드 위젯 시스템~~ → YAGNI, shadcn/ui Card 인라인 사용
...
| **M1.5** | 단계 1.5 | 수식·도형 렌더링 완료 — 단계 2 PDF 출력 진입 가능 |
...
| F018 | 역할별 대시보드 | 단계 2 |
| F019 | 학부모 리포트 | 단계 4 |
| F020 | LaTeX 수식 렌더링 | 단계 1.5 |
| F021 | 도형/그래프 렌더링 | 단계 1.5 |
...
| 단계 2 (성취기준 연동 + RBAC/대시보드) | 33% (2-1 RBAC + 2-2 성취기준 DB + 2-3 대시보드 완료, 1460 tests) | 🚧 진행 중 |
