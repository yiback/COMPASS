...
| **1-5** | **사용자 관리 CRUD [F009]** | **🚧 Step 4/5 완료** |
...
|------|------|----------|------|
| 1 | Zod 검증 스키마 (TDD) | 🟢 | ✅ 완료 |
| 2 | Server Actions (TDD) | 🔴 | ✅ 구현 + 학습 리뷰 완료 |
| 3 | DataTable + 목록 페이지 UI | 🟡 | ✅ 구현 + 학습 리뷰 완료 |
| 4 | 역할 변경 Dialog + 상세 Sheet | 🟡 | ✅ 구현 + 학습 리뷰 완료 |
| **5** | **메뉴 + 빌드 검증 + 학습 리뷰** | **-** | **⏳ 다음 작업** |
...
### 즉시: 1-5 Step 5 — 사이드바 메뉴 + 빌드 검증 + 최종 학습 리뷰

**계획**: `docs/plan/phase-1-step5-user-crud.md` Step 5 섹션

**구현 대상**:
- `src/lib/constants/menu.ts` — "사용자 관리" (`/admin/users`) 메뉴 항목 추가
- 아이콘: `Users` (lucide-react), 위치: 학원 관리 아래
