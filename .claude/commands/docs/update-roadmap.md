...
7. 진행 상황 통계 업데이트
8. **`docs/plan/` Glob 검색으로 관련 파일 모두 탐색**
   - 상위 계획 문서 (예: `phase-1-step5-user-crud.md`)
   - **스텝별 세부 계획 문서 (예: `phase-1-step5-4-*.md`) ← 반드시 포함**
9. **탐색된 모든 문서 업데이트** (누락 없이)
10. 변경 사항 확인 메시지 출력 (업데이트된 파일 목록 명시)
...
#### docs/plan/ 파일 네이밍 규칙 (IMPORTANT)

```
상위 계획 문서:       phase-{단계}-{설명}.md
                     예) phase-1-step5-user-crud.md

스텝별 세부 계획 문서: phase-{단계}-step{번호}-{번호}-{설명}.md
                     예) phase-1-step5-4-role-dialog-detail-sheet.md
                         phase-1-step5-3-datatable-ui.md
                         phase-1-step5-2-server-actions.md
                         phase-1-step5-1-zod-schemas.md
```

**⚠️ 중요**: 스텝별 세부 계획 문서가 존재하면 반드시 함께 업데이트해야 한다.
업데이트 전 항상 `docs/plan/` 폴더를 Glob으로 검색하여 관련 파일을 모두 확인할 것.

#### 업데이트 대상 파일 탐색 순서

1. Glob(`docs/plan/**`)으로 전체 파일 목록 확인
2. 완료한 Task와 관련된 **모든** 파일 식별:
   - 상위 계획 문서 (예: `phase-1-step5-user-crud.md`)
   - 스텝별 세부 계획 문서 (예: `phase-1-step5-4-*.md`)
3. 각 파일을 **모두** 업데이트

#### 각 파일 업데이트 내용

1. **진행 상태 요약 테이블 업데이트**: 완료한 Step의 상태를 `⏸️` → `✅`로 변경
2. **Step 섹션 상태 업데이트**: `**상태**: ⏸️ pending` → `**상태**: ✅ completed`
3. **검증 기준 체크**: 해당 Step의 `- [ ]` 항목을 `- [x]`로 변경
4. **완료 요약 추가**: Step 섹션에 `**완료 요약**:` 블록 추가 (작업 내용 간략 기술)
5. **헤더 진행률 업데이트**: 문서 상단의 `> **진행률**: X/N Steps 완료 (XX%)` 갱신
6. **마지막 업데이트 날짜 갱신**: `> **마지막 업데이트**: YYYY-MM-DD`
7. **파일 구조 섹션**: 완료된 파일에 `[완료]` 표시 추가
8. **전체 Phase 완료 시**: 상단 `> **상태**: 🚧 진행 중` → `> **상태**: ✅ 완료`
...
- **스텝별 세부 계획 문서 (`phase-{단계}-step{번호}-{번호}-*.md`)가 존재하면 반드시 함께 업데이트** — 상위 문서만 업데이트하고 세부 문서를 누락하는 것은 규칙 위반
- 업데이트 전 반드시 Glob으로 `docs/plan/` 전체 파일 목록을 확인하여 관련 파일을 빠짐없이 탐색할 것
